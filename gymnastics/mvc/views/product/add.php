                ...
                <form class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
                    <div class="form-group <?= form_error('photo') ? 'has-error' : '' ?>" >
                        <label for="photo" class="col-sm-2 control-label">
                            <?= $this->lang->line("product_bundle") ?>
                        </label>
                        <div class="col-sm-6" style="display:inline-flex;">
                            <input type="checkbox" class="form-control" value="TRUE" name="bundleMark" id="bundleMark" style="width: 60px;"> <b style="margin: 10px;"><?= $this->lang->line("product_mark_bundle"); ?></b> 
                        </div>
                        <span class="col-sm-4">
                            <?php echo form_error('photo'); ?>
                        </span>
                    </div>
                    <div class="form-group" id="bundleProductDiv" style="display:none"></div>
                    ...
                    <div class="form-group <?= form_error('productsize') ? 'has-error' : '' ?>" >
                        ...
                        <div class="col-sm-6">
                            <input type="text" class="form-control" id="productsize" name="productsize" value="<?= set_value('productsize') ?>" >
                        </div>
                        <span class="col-sm-4 control-label">
                    ...
                    <div class="form-group <?= form_error('productcolor') ? 'has-error' : '' ?>" >
                        ...
                        <div class="col-sm-6">
                            <input type="text" class="form-control" id="productcolor" name="productcolor" value="<?= set_value('productcolor') ?>" >
                        </div>
                        <span class="col-sm-4 control-label">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $("#bundleMark").change(function () {
            if (this.checked) {
                $.ajax({
                    type: 'POST',
                    url: "<?= base_url('product/getProductForBundle') ?>",
                    data: "bundle=TRUE",
                    success: function (data) {
                        $("#bundleProductDiv").html(data);
                        $("#bundleProductDiv").css("display", "block");
                    }
                });
            } else {
                $("#bundleProductDiv").empty();
                $("#bundleProductDiv").css("display", "block");
            }
        });
    });
