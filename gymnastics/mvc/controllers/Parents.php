...
class Parents extends Admin_Controller {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                for ($i = 0; $i < count($rules); $i++) {
                  if($rules[$i]['field'] == "mother_name"){
                    $array[$rules[$i]['field']] =date("Y-m-d", strtotime($this->input->post($rules[$i]['field'])));
                  } else {
            ...
            if ($this->data['parents']) {
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        $this->data['allStudent'] = $this->student_m->get_order_by_student(array("parentID IS NULL" => null));
    ...
    public function parentAdd() {
        ...
        $this->session->set_flashdata('success', $this->lang->line('menu_success'));
        redirect(base_url("parents/edit/" . $this->input->post("parentID")));
    }

    public function remove() {
        $studentID = htmlentities(escapeString($this->uri->segment(3)));
        $parentID = htmlentities(escapeString($this->uri->segment(4)));
        if ((int) $studentID && (int) $parentID) {
            $array = array();
            $array['parentID'] = NULL;

            $this->student_m->update_student($array, $studentID);
            redirect(base_url("parents/edit/" . $parentID));
        } else {
