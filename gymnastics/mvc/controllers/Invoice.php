...
class Invoice extends Admin_Controller {
    ...
    public function index() {
        ...
        if ($usertypeID == 3) {
            ...
            if (count($student)) {
                $this->data['maininvoices'] = $this->maininvoice_m->get_maininvoice_by_studentID($student->studentID);
            ...
            if (count($students)) {
                ...
                $this->data['maininvoices'] = $this->maininvoice_m->get_maininvoice_by_studentID($studentArray);
                $this->data['grandtotalandpayment'] = $this->grandtotalandpaid($this->data['maininvoices'], $schoolyearID);
                $this->data["subview"] = "invoice/index";
                $this->load->view('_layout_main', $this->data);
            } else {
            ...
            $this->data['maininvoices'] = $this->maininvoice_m->get_maininvoice_with_student($schoolyearID);
            $this->data['grandtotalandpayment'] = $this->grandtotalandpaid($this->data['maininvoices'], $schoolyearID);
            $this->data["subview"] = "invoice/index";
            $this->load->view('_layout_main', $this->data);
        }
    ...
    public function view() {
        ...
        if ($usertypeID == 3) {
            ...
            if ((int) $id) {
                ...
                $getstudent = $this->student_m->get_single_student(array('studentID' => $studentID));
                if (count($getstudent)) {
            ...
            $id = htmlentities(escapeString($this->uri->segment(3)));
            if ((int) $id) {
                $this->data['maininvoice'] = $this->maininvoice_m->get_maininvoice_with_student_by_maininvoiceID($id, $schoolyearID);
                $this->data['invoices'] = $this->invoice_m->get_order_by_invoice(array('maininvoiceID' => $id));
    ...
    public function check_valid_paymentmethod() {
        $usertypeID = $this->session->userdata('usertypeID');
        $paymentMethod = ['cash', 'bank', 'Cash', 'Cheque', 'Paypal', 'Stripe', 'Payumoney', 'Voguepay'];
    ...
    public function unique_paymentmethod() {
        if ($this->input->post('paymentmethodID') === '0') {
            ...
            if ($this->input->post('paymentmethodID') == 'cash' || $this->input->post('paymentmethodID') == 'bank') {
                return TRUE;
            } elseif ($this->input->post('paymentmethodID') == 'Cash' || $this->input->post('paymentmethodID') == 'Cheque') {
                return TRUE;
            ...
            if ((int) $id) {
                ...
                if (count($maininvoice)) {
                    if ($maininvoice->maininvoicestatus != 2) {
                        $this->data['student'] = $this->student_m->get_single_student(array('studentID' => $maininvoice->maininvoicestudentID));
                        $this->data['studentprofile'] = $this->student_m->get_single_student(array('studentID' => $maininvoice->maininvoicestudentID));
                        if (count($this->data['student'])) {
                            ...
                            if ($usertypeID == 3) {
                                ...
                                $fetchStudent = pluck($getStudents, 'srstudentID', 'srstudentID');
                                if (count($fetchStudent)) {
                                    if (in_array($this->data['student']->studentID, $fetchStudent)) {
                                        $f = TRUE;
                                    }
                                }
                            } else {
                            ...
                            if ($f) {
                                ...
                                $this->data['class'] = $this->classes_m->general_get_single_classes(array('classesID' => $this->data['student']->classesID));
                                $this->data['section'] = $this->section_m->general_get_single_section(array('sectionID' => $this->data['student']->sectionID));

                                $this->data['invoices'] = $this->invoice_m->get_order_by_invoice(array('maininvoiceID' => $id, 'deleted_at' => 1));

                                $this->data['feetypes'] = pluck($this->feetypes_m->get_feetypes(), 'feetypes', 'feetypesID');

                                $this->data['invoicepaymentandweaver'] = $this->paymentdue($maininvoice, $schoolyearID, $this->data['student']->studentID);
                                        ...
                                        if ($this->input->post('paymentmethodID') == 'cash' || $this->input->post('paymentmethodID') == 'bank') {
                                            if (count($this->data['invoices'])) {
                                                ...
                                                $globalpayment = array(
                                                    ...
                                                    'sectionID' => $this->data['student']->sectionID,
                                                    'studentID' => $maininvoice->maininvoicestudentID,
                                                    'clearancetype' => 'partial',
                                                    'invoicename' => $this->data['student']->registerNO . '-' . $this->data['student']->name,
                                                    'invoicedescription' => '',
                                                    'paymentyear' => date('Y'),
                                                ...
                                                foreach ($this->data['invoices'] as $key => $invoice) {
                                                    if ($invoice->paidstatus != 2) {
                                                        ...
                                                        $paymentArray = array(
                                                            ...
                                                            'paymenttype' => $this->input->post('paymentmethodID'),
                                                        ...
                                                        if ($this->input->post("transactionNumber") != null && $this->input->post("transactionNumber") != '') {
                                                            $paymentArray['transactionNumber'] = $this->input->post("transactionNumber");
                                                        }
            ...
            if ((int) $globalpaymentID && (int) $maininvoiceID) {
                ...
                if (count($maininvoice) && count($globalpayment)) {
                    ...
                    if ($usertypeID == 3) {
                        $getstudent = $this->student_m->get_single_student(array('studentID' => $studentID));
                        if (count($getstudent)) {
                            if ($getstudent->studentID == $userID) {
                                $f = TRUE;
                            }
                        }
                    } elseif ($usertypeID == 4) {
                ...
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        if (((int) $studentID || $studentID == 0) && (int) ($classesID)) {
                            ...
                            $getstudents = $this->student_m->get_single_student(array('studentID' => $studentID));
                            ...
                            if (count($getstudents)) {
                                ...
                                $invoiceMainArray[] = array(
                                    ...
                                    'paidrecurring' => $this->input->post('paiymentrecuring'),
                                ...
                                $globalPaymentArray[] = array(
                                    ...
                                    'invoicename' => $getstudent->registerNO . '-' . $getstudent->name,
                                    'invoicedescription' => '',
                                    'paymentyear' => date('Y'),
                                    'schoolyearID' => $schoolyearID,
                                );
                    ...
                    if (count($maininvoice)) {
                        ...
                        if (count($this->data['student'])) {
                            ...
                            if (count($this->data['invoices'])) {
                                $invoicepaymentandweaver = $this->data['invoicepaymentandweaver'];
                                $globalpayment = array(
                                    'classesID' => $maininvoice->maininvoiceclassesID,
                                    'sectionID' => '',
                                    'studentID' => $maininvoice->maininvoicestudentID,
                                    'clearancetype' => 'partial',
                                    'invoicename' => $this->data['student']->registerNO . '-' . $this->data['student']->name,
    ...
    public function generateHash($array) {
        ...
        if (
                empty($array['key']) || empty($array['txnid']) || empty($array['amount']) || empty($array['firstname']) || empty($array['email']) || empty($array['phone']) || empty($array['productinfo']) || empty($array['surl']) || empty($array['furl']) || empty($array['service_provider'])
        ) {
            return false;
        } else {
            $hash = '';
    ...
    public function verifyVoguePayPayment($transaction_id) {
        ...
        if (!$details && $this->debug == true) {
            $this->debug_msg[] = "Failed Getting Transaction Details - [Called In verifyPayment()]";
        }
        if ($details->total < 1)
            return json_encode(array("state" => "error", "msg" => "Invalid Transaction"));
        if ($details->status != 'Approved')
            return json_encode(array("state" => "error", "msg" => "Transaction {$details->status}"));
    ...
    public function getVoguePayPaymentDetails($transaction_id, $type = "json") {
        ...
        if (curl_errno($ch) && $this->debug == true) {
            $this->debug_msg[] = curl_error($ch) . " - [Called In getPaymentDetails() CURL]";
        }
        $output = curl_exec($ch);
        curl_close($ch);
