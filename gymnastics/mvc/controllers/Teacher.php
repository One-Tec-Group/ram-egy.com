...
class Teacher extends Admin_Controller {
    ...
    public function add() {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                do {
                    $registerNumber = $this->teacher_m->get_next_registerNO();
                    $teacherRegisterNumber = $this->teacher_m->get_single_teacher(array('teacherRegisterNumber' => $registerNumber));
                    if (count($teacherRegisterNumber)) {
                        $x = FALSE;
                    } else {
                        $x = TRUE;
                    }
                ...
                $array["teacherRegisterNumber"] = $this->input->post("registerNO");
    ...
    public function unique_registerNO() {
        $id = htmlentities(escapeString($this->uri->segment(3)));
        if ((int) $id) {
            $teacherRegisterNumber = $this->teacher_m->get_single_teacher(array('teacherRegisterNumber' => $this->input->post("registerNO")));
            if (count($teacherRegisterNumber)) {
                $this->form_validation->set_mess("unique_registerNO", "The %s is already exists.");
                return FALSE;
            }
            return TRUE;
        } else {
