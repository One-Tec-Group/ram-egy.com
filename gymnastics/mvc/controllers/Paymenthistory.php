...
*/
	...
	public function index() {
		...
		if($usertypeID == 3) {
			$this->data['payments'] = $this->payment_m->get_payment_with_student_by_studentID($userID, $schoolyearID);
			...
			$students = $this->student_m->get_order_by_student(array('parentID' => $userID, 'schoolyearID' => $schoolyearID));
			if(count($students)) {
				$studentArray = pluck($students, 'studentID');
				$this->data['payments'] = [];
				$this->data['payments'] = $this->payment_m->get_payment_with_student_by_studentID($studentArray, $schoolyearID);
			...
			$this->data['payments'] = $this->payment_m->get_payment_with_student($schoolyearID);
			$this->data["subview"] = "paymenthistory/index";
			$this->load->view('_layout_main', $this->data);
		}
	}
