...
class student_m extends MY_Model {
    ...
    public function get_next_registerNO() {
        $registerNumberInt = $this->db->select("max(studentID) as maxid")->from("student")->get()->row()->maxid;
        $registerNumber = "STD00" . ($registerNumberInt + 1);
        return $registerNumber;
    }

    public function get_notes($id) {
        $query = $this->db->query("SELECT * FROM `notes` WHERE `studentID` = $id");
        return $query->result();
    }

    public function add_note($noteData) {
        return $this->db->insert("notes", $noteData);
    }
