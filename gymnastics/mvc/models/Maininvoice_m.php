...
class Maininvoice_m extends MY_Model {
	...
	public function get_maininvoice_with_student($schoolyearID = NULL) {
		$this->db->select('*');
		$this->db->from($this->_table_name);
	...
	public function get_maininvoice_by_studentID($studentID) {
		$this->db->select('*');
		$this->db->from($this->_table_name);
		$this->db->join('student', 'student.studentID = maininvoice.maininvoicestudentID AND student.classesID = maininvoice.maininvoiceclassesID', 'LEFT');
		$this->db->where('maininvoice.maininvoicedeleted_at', 1);
		$this->db->order_by('maininvoice.maininvoiceID', 'desc');
		$query = $this->db->get();
		return $query->result();
	...
	public function get_maininvoice_with_student_by_maininvoiceID($invoiceID, $schoolyearID = NULL) {
		$this->db->select('*');
		$this->db->from($this->_table_name);
		$this->db->join('student', 'student.studentID = maininvoice.maininvoicestudentID AND student.schoolyearID = maininvoice.maininvoiceschoolyearID', 'LEFT');
		$this->db->where('maininvoice.maininvoiceID', $invoiceID);
