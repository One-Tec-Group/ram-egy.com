...
<div class="mb-mail-date pull-right"><?= time_ago($v_comment->comment_datetime) ?>
            ...
            <div class="mb-mail-meta">
                ...
                <div class="mb-mail-album pull-left">
                    ...
                    if (!empty($uploaded_file)) {
                        foreach ($uploaded_file as $v_files) {
                            if (!empty($v_files)) {
                                ?>
                                <a href="<?= base_url() ?>admin/opportunities/download_files/<?= $v_files->fileName . '/' . true ?>">
                ...
                <?php $this->load->view('admin/opportunities/comments_reply', array('comment_reply_details' => $comment_reply_details)) ?>
                <div class="reply__" id="reply_<?php echo $v_comment->task_comment_id ?>">
                    <form id="<?php echo $comment_reply_type; ?>-comment-form-<?php echo $v_comment->task_comment_id ?>"
                          action="<?php echo base_url() ?>admin/opportunities/save_comments_reply/<?php
                          if (!empty($v_comment->task_comment_id)) {
                              echo $v_comment->task_comment_id;
                          }
                          ?>" method="post">
                        <input type="hidden" name="opportunities_id" value="<?php
                        if (!empty($v_comment->opportunities_id)) {
                            echo $v_comment->opportunities_id;
                        }
                        ?>" class="form-control">
