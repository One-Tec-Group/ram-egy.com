...
<div class="mb-mail-date pull-right"><?= time_ago($v_comment->comment_datetime) ?>
                <?php if ($v_comment->user_id == $this->session->userdata('user_id')) { ?>
                    <?php echo ajax_anchor(base_url("admin/leads/delete_comments/" . $v_comment->task_comment_id), "<i class='text-danger fa fa-trash-o'></i>", array("class" => "", "title" => lang('delete'), "data-fade-out-on-success" => "#" . $comment_type . "-comment-form-container-" . $v_comment->task_comment_id)); ?>
                <?php } ?>
            </div>
            <div class="mb-mail-meta">
                ...
                <div class="mb-mail-album pull-left">
                    ...
                    if (!empty($uploaded_file)) {
                        foreach ($uploaded_file as $v_files) {
                            if (!empty($v_files)) {
                                ?>
                                <a href="<?= base_url() ?>admin/leads/download_files/<?= $v_files->fileName . '/' . true ?>">
                ...
                <?php $this->load->view('admin/leads/comments_reply', array('comment_reply_details' => $comment_reply_details)) ?>
                <div class="reply__" id="reply_<?php echo $v_comment->task_comment_id ?>">
                    <form id="<?php echo $comment_reply_type; ?>-comment-form-<?php echo $v_comment->task_comment_id ?>"
                          action="<?php echo base_url() ?>admin/leads/save_comments_reply/<?php
                          if (!empty($v_comment->task_comment_id)) {
                              echo $v_comment->task_comment_id;
                          }
                          ?>" method="post">
                        <input type="hidden" name="leads_id" value="<?php
                        if (!empty($v_comment->leads_id)) {
                            echo $v_comment->leads_id;
                        }
                        ?>" class="form-control">
