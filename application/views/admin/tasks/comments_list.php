...
if (!empty($comment_details)) {
    foreach ($comment_details as $key => $v_comment) {
        ...
        ?>
<div class="mb-mails col-sm-12" id="<?php echo $comment_type . "-comment-form-container-" . $v_comment->task_comment_id ?>"><img alt="Mail Avatar" src="<?php echo base_url() . $profile_info->avatar ?>"
            ...
            <div class="mb-mail-date pull-right"><?= time_ago($v_comment->comment_datetime) ?>
                <?php if ($v_comment->user_id == $this->session->userdata('user_id')) { ?>
                    <?php echo ajax_anchor(base_url("admin/tasks/delete_task_comments/" . $v_comment->task_comment_id), "<i class='text-danger fa fa-trash-o'></i>", array("class" => "", "title" => lang('delete'), "data-fade-out-on-success" => "#" . $comment_type . "-comment-form-container-" . $v_comment->task_comment_id)); ?>
                <?php } ?>
            </div>
            <div class="mb-mail-meta">
                <div class="pull-left">
                    <div class="mb-mail-from"><a
                            href="<?= base_url() ?>admin/user/user_details/<?= $v_comment->user_id ?>"> <?= ($profile_info->fullname) ?></a>
                    </div>
                </div>
                <div
                ...
                <div class="mb-mail-album pull-left">
                    ...
                    if (!empty($uploaded_file)) {
                        foreach ($uploaded_file as $v_files) {
                            if (!empty($v_files)) {
                                ?>
                                <a href="<?= base_url() ?>admin/tasks/download_files/<?=  $v_files->fileName . '/' . true ?>">
                ...
                <?php $this->load->view('admin/tasks/comments_reply', array('comment_reply_details' => $comment_reply_details)) ?>
                <div class="reply__" id="reply_<?php echo $v_comment->task_comment_id ?>">
                    <form id="<?php echo $comment_reply_type; ?>-comment-form-<?php echo $v_comment->task_comment_id ?>"
                          action="<?php echo base_url() ?>admin/tasks/save_comments_reply/<?php
                          if (!empty($v_comment->task_comment_id)) {
                              echo $v_comment->task_comment_id;
                          }
                          ?>" method="post">
                        <input type="hidden" name="task_id" value="<?php
                        if (!empty($v_comment->task_id)) {
                            echo $v_comment->task_id;
                        }
                        ?>" class="form-control">
