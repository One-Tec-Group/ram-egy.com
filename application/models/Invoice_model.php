...
class Invoice_Model extends MY_Model
{
    ...
    public function get_invoice_total_amount($invoice_id)
    {
        ...
        if($invoice_info->adjustment > 0 && $invoice_info->decrease > 0 ){
        //  echo "cd";

          return (($invoice_cost - $discount) + $tax + $invoice_info->adjustment - $invoice_info->decrease);

        }elseif($invoice_info->adjustment > 0 && $invoice_info->decrease <= 0 ){
        //  echo "trg";

          return (($invoice_cost - $discount) + $tax + $invoice_info->adjustment);

        }elseif($invoice_info->adjustment <= 0 && $invoice_info->decrease > 0 ){
        //  echo "l0k";

          return (($invoice_cost - $discount) + $tax - $invoice_info->decrease);

        } else{
      //    echo "dd";

          return (($proposal_cost - $discount) + $tax + $invoice_info->decrease + $invoice_info->adjustment );
        }
...
//            }
    ...
    public function send_invoice_sms($invoices_id)
    {
        ...
        if (!empty($client_info)) {
            $currency = $this->client_currency_sambol($client_info->client_id);;
