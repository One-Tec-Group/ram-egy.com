...
class Datatables extends MY_Model
{
    ...
    function make_query()
    {
        if ($this->table == 'tbl_transactions') {
        ...
        } elseif (!is_null($this->join_table) && !is_null($this->join_where)) {
            for ($i = 0; $i < count($this->join_table); $i++) {
                $this->db->join($this->join_table[$i], $this->join_where[$i], 'left');
            }
        }
    ...
    function get_filtered_data($where = null, $where_in = null)
    {
        ...
        if ($role == 3 || !empty($userid)) {
            ...
            if (!empty($this->db->field_exists('permission', $this->table))) {
                ...
                $this->db->where($this->table . '.permission REGEXP', $sq, false);
                $this->db->or_where(array($this->table . '.permission' => 'all'));
                $this->db->group_end(); //close bracket
            }
        }
    ...
    function get_all_data($where = null, $where_in = null)
    {
        $this->db->select("*");
        $this->db->from($this->table);
        if (!empty($where) && is_numeric($where)) {
