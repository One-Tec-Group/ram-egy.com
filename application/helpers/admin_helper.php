...
function client_currency($id)
{
    ...
    $currency_info = $CI->invoice_model->client_currency_sambol($id);;
    if (empty($currency_info)) {
...
function admin_head()
{
    if (!empty(admin())) {
        return true;
    } elseif (!empty(is_department_head())) {
        return true;
    } else {
        return false;
    }
...
function send_clock_email($type)
{
    if (!empty(config_item('send_clock_email'))) {
        ...
        $IP = $CI->input->ip_address();
        $CI->load->model('settings_model');
        $email_template = $CI->settings_model->check_by(array('email_group' => $type), 'tbl_email_templates');
...
function merge_proposal_template($proposals_id = null)
{
    ...
    if (!empty($proposals_id)) {
        ...
        if (!empty($client_info)) {
            $currency = $CI->proposal_model->client_currency_sambol($client_info->client_id);
        } else {
...
function merge_estimate_template($estimates_id = null)
{
    ...
    if (!empty($estimates_id)) {
        ...
        if (!empty($client_info)) {
            $currency = $CI->estimates_model->client_currency_sambol($client_info->client_id);
        } else {
...
function merge_invoice_template($invoices_id = null, $type = null, $type_id = null)
{
    ...
    if (!empty($invoices_id)) {
        ...
        if (!empty($client_info)) {
            $currency = $CI->invoice_model->client_currency_sambol($client_info->client_id);
        } else {
