...
class Proposals extends Admin_Controller
{
    ...
    public function pdf_proposals($id){
                include('application/helpers/mpdf/vendor/autoload.php');
                $mpdf = new \Mpdf\Mpdf();
                //$mpdf->SetHTMLFooter('<div style="text-align:center"> {PAGENO}/{nbpg} </div>');

                $data['proposals_info'] = $this->invoice_model->check_by(array('proposals_id' => $id), 'tbl_proposals');
                $data['title'] = "proposals PDF"; //Page title
                ...
                $mpdf->WriteHTML($viewfile);
               //  $stylesheet = file_get_contents(base_url().'assets/css/app.min.css'); // external css
               //  $mpdf->WriteHTML($stylesheet,1);
              //  var_dump($viewfile);
               //exit();
                // $mpdf->AddPage();
                // $mpdf->use_kwt = true;
                $mpdf->Output('proposals# ' . $data['proposals_info']->reference_no.'.pdf','D'); // opens in browser
    ...
    public function save_proposals($id = NULL)
    {
        ...
        if (!empty($created) || !empty($edited) && !empty($id)) {
            $data = $this->proposal_model->array_from_post(array('reference_no', 'discount_type', 'discount_percent', 'user_id','decrease', 'adjustment', 'discount_total', 'show_quantity_as'));
    ...
    function cloned_proposals($id)
    {
        ...
        if (!empty($can_edit) && !empty($edited)) {
            ...
            $new_invoice = array(
                ...
                'decrease' => $invoice_info->decrease,
    ...
    {
        $data = $this->proposal_model->array_from_post(array('reference_no', 'client_id', 'project_id', 'discount_type', 'discount_percent', 'user_id','decrease', 'adjustment', 'discount_total', 'show_quantity_as'));
