...
class Estimates extends Admin_Controller
{
    ...
    function client_change_data($customer_id, $current_invoice = 'undefined')
    {
        if ($this->input->is_ajax_request()) {
            $data = array();
            $data['client_currency'] = $this->estimates_model->client_currency_sambol($customer_id);
    ...
    function send_estimates_email($estimates_id, $row = null)
    {
        if (!empty($row)) {
            ...
            if (!empty($client_info)) {
                ...
                $currency = $this->estimates_model->client_currency_sambol($client_info->client_id);;
            } else {
                $client = '-';
