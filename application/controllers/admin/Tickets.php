...
class Tickets extends Admin_Controller
{
        ...
        if ($action == 'edit_tickets' || $action == 'project_tickets') {
            ...
            if (!empty($project_id)) {
                ...
                if ($project_info->permission == 'all') {
                    $data['permission_user'] = $this->tickets_model->allowad_user('57');
                } else {
                    $data['permission_user'] = $this->tickets_model->permitted_allowad_user($project_info->permission);
                }
            }
            $data['active'] = 2;
        } else {
    ...
    function notify_ticket_reopened($id)
    {
        ...
        $email_template = $this->tickets_model->check_by(array('email_group' => 'ticket_reopened_email'), 'tbl_email_templates');
