...
//        redirect('admin/transactions/deposit');
            ...
            if (is_numeric($id)) {
                ...
                if ($id == 'project_expense') {
                    ...
                    if ($project_info->permission == 'all') {
                        $data['permission_user'] = $this->transactions_model->allowad_user('57');
                    } else {
                        $data['permission_user'] = $this->transactions_model->permitted_allowad_user($project_info->permission);
                    }
                }
            }
        } else {
    ...
    public function expenseList($filterBy = null, $type = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->datatables->column_order = array('name', 'date', 'amount', 'notes', 'tbl_accounts.account_name', 'status', 'reference');
            $this->datatables->column_search = array('name', 'date', 'amount', 'notes', 'tbl_accounts.account_name', 'status', 'reference');
            $this->datatables->order = array('transactions_id' => 'desc');
            $where = null;
            if (empty($type)) {
            ...
            foreach ($fetch_data as $_key => $v_expense) {
                ...
                $sub_array = array();
                $name = null;
                $name .= '<a data-toggle="modal" data-target="#myModal" class="text-info" href="' . base_url() . 'admin/transactions/view_expense/' . $v_expense->transactions_id . '">' . (!empty($v_expense->name) ? $v_expense->name : '-') . '</a>';
                $sub_array[] = $name;

                $date = null;
                $date .= '<a data-toggle="modal" data-target="#myModal" class="text-info" href="' . base_url() . 'admin/transactions/view_expense/' . $v_expense->transactions_id . '">' . strftime(config_item('date_format'), strtotime($v_expense->date)) . '</a>';
                $sub_array[] = $date;
                ...
                if ($role == 1 || !empty($check_head)) {
                    ...
                    $e_status .= '     <span class="label label-' . $label . '">' . lang($status) . '</span>';
                }
                ...
                $action .= '<a class="btn btn-info btn-xs" data-toggle="modal" data-target="#myModal" href="' . base_url() . 'admin/transactions/view_expense/' . $v_expense->transactions_id . '"><span class="fa fa-list-alt"></span></a>' . ' ';
                if (!empty($can_edit) && !empty($edited)) {
