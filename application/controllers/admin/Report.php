    ...
    public function expenseList($id = null, $type = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($fetch_data as $_key => $v_expense) {
                ...
                $action .= '<a class="btn btn-info btn-xs" data-toggle="modal" data-target="#myModal" class="text-info" href="' . base_url() . 'admin/transactions/view_expense/' . $v_expense->transactions_id . '"><span class="fa fa-list-alt"></span></a>' . ' ';
                if (!empty($can_edit) && !empty($edited)) {
