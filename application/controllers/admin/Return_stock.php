...
class Return_stock extends Admin_Controller
{
    ...
    public function send_return_stock_sms($purchase_id)
    {
        $mobile = can_received_sms('purchase_confirmation_sms_number');
        if (!empty($mobile)) {
            $merge_fields = [];
            $merge_fields = array_merge($merge_fields, merge_return_stock_template($purchase_id));
            $this->sms->send(SMS_RETURN_STOCK, $mobile, $merge_fields);
        }
        return true;
    }
...
The " . config_item('company_name') . " Team </p > ";
    ...
    public function attach_pdf($id)
    {
        ...
        $result = pdf_create($html, slug_it(lang('return_stock') . '_' . $data['return_stock_info']->reference_no), 1, null, true);
        return $result;
    }
    ...
    public function pdf_return_stock($id)
    {
        ...
        pdf_create($viewfile, slug_it(lang('return_stock') . '# ' . $data['return_stock_info']->reference_no));
    }
