...
class Items extends Admin_Controller
{
    ...
    public function itemsList($group_id = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->datatables->column_order = array('item_name','item_code', 'item_desc', 'hsn_code', 'quantity', 'unit_cost', 'unit_type');
            $this->datatables->column_search = array('item_name', 'item_desc','item_code','item_color' ,'hsn_code', 'quantity', 'unit_cost', 'unit_type');
            ...
            foreach ($fetch_data as $_key => $v_items) {
                ...
                $sub_array[] = $v_items->item_code;


if(!empty($v_items->item_color)){
   $sub_array[] = '<span style="background-color:'.$v_items->item_color.'" class="color-tag clickable" data-color="'.$v_items->item_color.'"></span>'.$v_items->item_color;
}else{
   $sub_array[] = $v_items->item_color;
}
    ...
    public function saved_items($id = NULL)
    {
        ...
        $data = $this->items_model->array_from_post(array('item_name','item_code','item_color','item_desc', 'hsn_code', 'unit_cost', 'unit_type', 'customer_group_id', 'quantity'));
