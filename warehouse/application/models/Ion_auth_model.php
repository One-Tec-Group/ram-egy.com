...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password,api_key, active,first_name,last_name, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	**/
	...
	public function set_data_in_session($email){
		...
		if($query->num_rows() > 0){
			...
			$data = array(
				...
				'forgotten_password_code' => $result->forgotten_password_code,
				'forgotten_password_time' => $result->forgotten_password_time,
				'remember_code' => $result->remember_code,
	...
	public function update_theme($theme){
		...
		$this->db->update('users',array('theme'=>$theme));
	}
	public function getTheme(){
		$this->db->where('id',$this->session->userdata('user_id'));
		return $this->db->get('users')->row()->theme;
	}
	public function currentMonthSales(){
		$result = $this->db->select('sum(total) as total')
	...
	public function profit(){
		$sales = $this->db->select('s.sales_id,si.product_id,si.quantity')
						   ->from('sales s')
		...
		/*foreach  ($sales1 as $row) {
			foreach ($total as $key => $value) {
				if($key == $row->sales_id){
					if($row->discount_type == "Fixed" && $total[$row->sales_id] > $row->amount){
						$total[$row->sales_id] -= $row->discount_value;
					}
					else if($row->discount_type == "Percentage"){
						$total[$row->sales_id]  = $total[$row->sales_id] - (($total[$row->sales_id] * $row->discount_value) / 100);
					}
				}
			}
		}*/
	...
	public function daySales(){
		$result = $this->db->query('SELECT s.sales_id, sum(s.total) AS total, date FROM sales s GROUP BY s.date');
		return  $result->result();
	}
	public function daySalesReturn(){
		$result = $this->db->query('SELECT s.id, sum(s.total) AS total, date FROM sales_return s GROUP BY s.date');
		return  $result->result();
	}
	...
	public function allReports(){
	    ...
	    /*  Products  */
		$data['todayProduct'] = $this->db->select('count(*) as item')->where('date',date("Y-m-d"))->get('products')->result();
		$data['weekProduct'] = $this->db->select('count(*) as item')->where('date BETWEEN "'.$from.'" AND "'.$to.'"')->get('products')->result();
		$data['monthProduct'] = $this->db->select('count(*) as item')->like('date',date('Y-m'))->get('products')->result();
		$data['yearProduct'] = $this->db->select('count(*) as item')->like('date',date('Y'))->get('products')->result();
		$data['allProduct'] = $this->db->select('count(*) as item')->get('products')->result();

		/*  Purchase */
		$data['todayPurchase'] = $this->db->select('sum(pi.quantity) as item, sum(pr.cost*pi.quantity) as value')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->join('products pr','pr.product_id = pi.product_id')
								  ->where('p.date',date("Y-m-d"))
								  // ->where('pr.user_id',$this->session->userdata('user_id'))
								  ->where('p.delete_status',0)
								  ->get()->result();
	
		$data['weekPurchase'] = $this->db->select('sum(pi.quantity) as item, sum(pr.cost*pi.quantity) as value')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->join('products pr','pr.product_id = pi.product_id')
								  // ->where('pr.user_id',$this->session->userdata('user_id'))
		...
		$data['monthPurchase'] = $this->db->select('sum(pi.quantity) as item, sum(pr.cost*pi.quantity) as value')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->join('products pr','pr.product_id = pi.product_id')
								  // ->where('pr.user_id',$this->session->userdata('user_id'))
								  ->where('p.delete_status',0)
								  ->like('p.date',date('Y-m'))
								  ->get()->result();
		$data['yearPurchase'] = $this->db->select('sum(pi.quantity) as item, sum(pr.cost*pi.quantity) as value')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->join('products pr','pr.product_id = pi.product_id')
								  // ->where('pr.user_id',$this->session->userdata('user_id'))
								  ...
								  ->like('p.date',date('Y'))
								  ->get()->result();
		$data['allPurchase'] = $this->db->select('sum(pi.quantity) as item, sum(pr.cost*pi.quantity) as value')
								  ->from('purchases p')
								  ->join('purchase_items pi','p.purchase_id = pi.purchase_id')
								  ->join('products pr','pr.product_id = pi.product_id')
								  // ->where('pr.user_id',$this->session->userdata('user_id'))
		...
		/* Sales */
		$data['todaySales'] = $this->db->select('sum(si.quantity) as item,sum(pr.price*si.quantity) as value')
								  ->from('sales s')
								  ...
								  ->get()->result();
		$data['weekSales'] = $this->db->select('sum(si.quantity) as item,sum(pr.price*si.quantity) as value')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->join('products pr','pr.product_id = si.product_id')
								  // ->where('pr.user_id',$this->session->userdata('user_id'))
		...
		$data['monthSales'] = $this->db->select('sum(si.quantity) as item,sum(pr.price*si.quantity) as value')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->join('products pr','pr.product_id = si.product_id')
								  // ->where('pr.user_id',$this->session->userdata('user_id'))
								  ->where('s.delete_status',0)
								  ->like('s.date',date('Y-m'))
								  ->get()->result();
		$data['yearSales'] = $this->db->select('sum(si.quantity) as item,sum(pr.price*si.quantity) as value')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->join('products pr','pr.product_id = si.product_id')
								  // ->where('pr.user_id',$this->session->userdata('user_id'))
								  ...
								  ->like('s.date',date('Y'))
								  ->get()->result();
		$data['allSales'] = $this->db->select('sum(si.quantity) as item,sum(pr.price*si.quantity) as value')
								  ->from('sales s')
								  ->join('sales_items si','s.sales_id = si.sales_id')
								  ->join('products pr','pr.product_id = si.product_id')
								  // ->where('pr.user_id',$this->session->userdata('user_id'))
		...
		$data['warehouse'] = $this->db->select('*')
								  ->from('warehouse w')
								  ->where('w.delete_status',0)
								  ->get()->result();

		$data['product'] = $this->db->select('p.product_id')
									->from('warehouse w')
									->join('warehouses_products wp','w.warehouse_id = wp.warehouse_id')
									...
									->get()
									->result();
		$data['warehouse_product'] = $this->db->select('sum(wp.quantity) as warehouse_item,sum(wp.quantity*p.cost) as value')
											 ->from('warehouse w')
											 ->join('warehouses_products wp','w.warehouse_id = wp.warehouse_id')
											 ->join('products p','wp.product_id = p.product_id')
											 // ->where('p.user_id',$this->session->userdata('user_id'))
											 ...
											 ->get()
											 ->result();
		$data['total_sales'] = $this->db->select('sum(si.quantity*p.price) as total_sales')
										->from('sales s')
										->join('sales_items si','si.sales_id = s.sales_id')
		...
		$data['company_setting'] = $this->db->query('SELECT count(*) as count FROM company_settings')->row();
		$data['category']  = $this->db->query('SELECT count(*) as count FROM category')->row();
		$data['discount']  = $this->db->query('SELECT count(*) as count FROM discount')->row();
		$data['branch']    = $this->db->query('SELECT count(*) as count FROM branch')->row();
		$data['brand']     = $this->db->query('SELECT count(*) as count FROM brand')->row();
		$data['warehouse_count'] = $this->db->query('SELECT count(*) as count FROM warehouse WHERE warehouse.delete_status = 0')->row();
		$data['uqc']       = $this->db->query('SELECT count(*) as count FROM uqc')->row();
		$data['product']  = $this->db->query('SELECT count(*) as count FROM products WHERE products.delete_status = 0')->row();
		$data['supplier'] = $this->db->query('SELECT count(*) as count FROM suppliers WHERE suppliers.delete_status = 0')->row();
		$data['customer']  = $this->db->query('SELECT count(*) as count FROM customer WHERE customer.delete_status = 0')->row();
		$data['biller']    = $this->db->query('SELECT count(*) as count FROM biller WHERE biller.delete_status = 0')->row();
		// $data['demo']	   = $this->db->get('demo')->row()->status;
		// echo "<pre>";
		// print_r($data);
		// exit;
		return $data;
	...
	public function getWarehouseData($id){
		...
		$data['product'] = $this->db->select('p.product_id')
									...
									->get()
									->result();
		$data['warehouse_product'] = $this->db->select('count(p.product_id) as products,sum(wp.quantity) as warehouse_item,sum(wp.quantity*p.cost) as value')
											 ->from('warehouse w')
											 ->join('warehouses_products wp','w.warehouse_id = wp.warehouse_id')
											 ->join('products p','wp.product_id = p.product_id')
											 ->where('w.warehouse_id',$id)
	...
	public function clearDemoData(){
		$this->db->truncate('account_payments');
		$this->db->truncate('account_receipts');
		$this->db->truncate('bank_account');
		$this->db->truncate('bank_account_deposit');
		$this->db->truncate('bank_account_transfer');
		$this->db->truncate('biller');
		$this->db->truncate('branch');
		$this->db->truncate('brand');
		$this->db->truncate('category');
		$this->db->truncate('company_settings');
		$this->db->truncate('credit_debit_note');
		$this->db->truncate('customer');
		$this->db->truncate('discount');
		$this->db->truncate('email_setup');
		$this->db->truncate('expense');
		$this->db->truncate('expense_category');
		$this->db->truncate('income_expence_category');
		$this->db->truncate('invoice');
		$this->db->truncate('ledger');
		$this->db->truncate('log');
		$this->db->truncate('payment');
		$this->db->truncate('payment_method');
		$this->db->truncate('payment_term');
		$this->db->truncate('products');
		$this->db->truncate('purchases');
		$this->db->truncate('purchase_items');
		$this->db->truncate('purchase_return');
		$this->db->truncate('purchase_return_items');
		$this->db->truncate('quotation');
		$this->db->truncate('quotation_items');
		$this->db->truncate('sales');
		$this->db->truncate('sales_items');
		$this->db->truncate('sales_return');
		$this->db->truncate('sale_return_items');
		$this->db->truncate('sub_category');
		$this->db->truncate('suppliers');
		$this->db->truncate('tax');
		$this->db->truncate('transaction_detail');
		$this->db->truncate('transaction_header');
		$this->db->truncate('transfer');
		$this->db->truncate('transfer_items');
		$this->db->truncate('warehouse');
		$this->db->truncate('warehouses_products');
		$this->db->truncate('warehouse_management');
		$this->db->update('demo',array('status'=>1));
	}

	public function updateBiller($id,$biller)
	{
		$this->db->where('user_id',$id);
		if($this->db->update('biller',$biller)){
			return TRUE;
		}
		else{
			return FALSE;
