<div id="tax_model" class="modal fade" role="dialog">
  <div class="modal-dialog modal-lg">
    ...
    <div class="modal-content">
         ...
         <form role="form" id="form" method="post" action="<?php echo base_url('tax/addTax');?>">
                ...
                <div class="col-sm-6">
                  ...
                  <div class="form-group calculate_on">
                    ...
                    <div class="input-group">
                      <input class="form-control text-right" type="text" id="calculate_on" name="calculate_on" value="100" placeholder="100">
      ...
      <div class="modal-footer">
        <button id="add_tax" class="btn btn-info btn-flat pull-left" data-dismiss="modal">ADD</button>
      </div>
    </div>
  </div>
</div>
...
//sales tax value validation complite.
      ...
      $.ajax({
        url:"<?php echo base_url('tax/add_tax_ajax') ?>",
        dataType: 'JSON',
        method:'POST',
        data:{
          ...
          'calculate_on': $('#calculate_on').val(),
          'tax_value': $('#tax_value').val(),
          'purchase_tax_value': $('#purchase_tax_value').val(),
          'description': $('#description').val()
        },
        success:function(result){
          var data = result['data'];
          $('#tax').html('');
          $('#tax').append('<option value="">Select</option>');
          for(i=0;i<data.length;i++){
              $('#tax').append('<option value="' + data[i].tax_id + '">' + data[i].tax_name + '</option>');
          }
          $('#tax').val(result['id']).attr("selected","selected");
        }
      });
